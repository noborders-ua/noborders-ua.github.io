...
// Minified
gulp.task('js-lib', () => {
  return gulp.src(path.src_js + '/index.js').
    pipe(rollup({
      allowRealFiles: true,
      input: './' + path.src_js + '/index.js',
      output: {
        format: 'iife',
      },
    })).
...
gulp.task('watch', () => {
  ...
  gulp.watch(path.src_js + '/**/*.js', gulp.series('js-lib'));
});
...
gulp.task(
  'default',
  gulp.series('clean', 'vendor', gulp.parallel('js', 'js-lib', 'sass:minified', 'sass:expanded'), 'watch'),
);
